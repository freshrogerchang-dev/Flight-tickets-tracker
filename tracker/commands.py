...
/scan 起日 迄日 晚數 [@路線名稱]
                             改掃描區間，例：/scan 2027-03-01 2027-03-20 14 @台北-雪梨
                             不加 @路線名稱 就套用到第一條路線，用 /routes 查路線名稱
/scan 出發日 晚數 [@路線名稱]
                             只掃一天，例：/scan 2027-03-05 14 @台北-雪梨
/time ...                   /scan 的別名，用法完全一樣（含 @路線名稱），改日期名字比較好記
/nights 晚數                只改待幾晚
...
/run                        立刻查一輪
/report                     近 30 天最低價，每條路線分開列，各取最便宜的 3 筆
