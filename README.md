...
**黃金海岸(OOL) 在 `max_stops: 2` 下查不到任何結果。**

2026-09-17 第一次實跑，31 組查詢裡 OOL 的 15 組全部失敗：

```
TPE>OOL>TPE ...: fast-flights 查詢失敗: TypeError: 'NoneType' object is not subscriptable
```

錯誤發生在 fast-flights 內部，推測是該條件下 Google 回傳空結果而它沒處理。
同一時間雪梨 14/15 組正常，所以不是網路或設定問題。

目前的處理：把 OOL 拆成獨立路線並放寬到 `max_stops: 3`（雪梨維持 2，
它在 2 段下跑得很好，放寬只會引入又長又爛的行程）。**此方案尚未驗證**。

若 3 段仍然全掛，下一步改追**布里斯本 BNE**——開車約一小時到黃金海岸，
班次多得多，票價常常還更便宜：
