...
| `/scan 起日 迄日 晚數 [@路線名稱]` | 改掃描區間，例：`/scan 2027-03-01 2027-03-20 14 @台北-雪梨` |
| `/scan 出發日 晚數 [@路線名稱]` | 只掃一天，例：`/scan 2027-03-05 14 @台北-雪梨` |
| `/time ...` | `/scan` 的別名，用法完全一樣（含 `@路線名稱`），改日期用這個名字比較好記 |
| `/nights 晚數` | 只改待幾晚 |
...
| `/newroute 名稱 出發地 目的地 出發日 晚數 [門檻]` | **新增一整條新路線**（來回），例：`/newroute 台北-福岡 TPE FUK 2027-06-05 7 20000` |
| `/newoneway 名稱 出發地 目的地 出發日 [門檻]` | **新增一整條新路線**（單程），例：`/newoneway 台北-福岡 TPE FUK 2027-06-05 8000` |
| `/newmulti 名稱 出發地>目的地@日期 ... [門檻]` | **新增一整條多段行程路線**，例：`/newmulti 雪梨進布里斯本出 TPE>SYD@2027-06-05 BNE>TPE@2027-06-16` |
| `/delroute 名稱` | **刪除一整條路線**（不能刪到一條都不剩，多段行程也適用） |
| `/pause [@路線名稱]` | **暫停**這條路線：不再查價、不再通知，`routes.yaml` 裡的設定原封不動保留 |
| `/resume [@路線名稱]` | 恢復暫停中的路線 |
| `/chart [@路線名稱]` | 把近 90 天價格畫成趨勢圖，用 Telegram 傳一張 PNG 回來（需要設定 Telegram） |
| `/price 金額` | 改通知門檻 |
| `/drop 百分比` | 改跌價通知門檻 |
...
路線（`trip: multi`）不生效，那種要直接改 `legs`，或用 `/newmulti` 整條重建。

`/add` `/rm` 是改**一條路線裡的目的地**；`/newroute` `/newoneway` `/newmulti`
`/delroute` 是新增/刪除**整條路線**，兩組指令的作用範圍不一樣，別搞混了。
`/newroute` 建立來回行程、`/newoneway` 建立單程行程（都沿用 `defaults` 的
轉機上限等設定）；多段行程（不同城市進出）要用 `/newmulti`，每一段用
`出發地>目的地@日期` 表示、中間用空格分開，至少要兩段。

`/pause` 保留設定只是跳過查價和通知——旅程取消了但可能還會再排、或只是
暫時不想被吵，用這個比 `/delroute` 再重建划算。`routes.yaml` 裡也可以直接寫
`paused: true`。`/chart` 需要一段時間的價格歷史（至少兩次查價紀錄）才畫得出
趨勢，剛加的新路線要先等排程跑過幾次。
...
`.github/workflows/digest.yml` 另外每週一送一次「存活摘要」（目前設定 +
每條路線最新查到的價格），不管有沒有便宜票、不等 `/run` 的通知門檻——
`track.yml` 只有真的有便宜票或整輪查詢全部失敗才會推播，一個「活著但一直
沒找到便宜票」的追蹤器和一個「早就靜靜壞掉」的追蹤器，從手機上看起來一模
一樣，這個排程就是定期的「我還活著，最近看到這些價格」證明。想改頻率一樣
是改 cron；不想要的話整個檔案刪掉，或用 `workflow_dispatch` 手動觸發驗證。
...
| `chart 路線名稱` | 把某條路線的價格歷史畫成 PNG 趨勢圖存在本機（本機測試用；手機上用指令頻道的 `/chart`；需要 `pip install -e ".[chart]"`） |
| `digest` | 送一則存活摘要（設定＋每條路線最新價格）到所有通知管道，不等有便宜票才發（`digest.yml` 每週跑一次） |
...
**多段行程（`trip: multi`，進出不同城市）的價格是兩張單程票加起來，不是聯程票價。**
fast-flights 自己的多段行程查詢模式在解析 Google 回傳結果時會壞掉
（實測過的錯誤：`IndexError: list index out of range`），這是那個第三方套件
處理這種回傳格式的 bug，修不了它的內部實作。Kiwi 也幫不上忙——查過它的
`search-flight` API 規格，本來就只能查「同一個出發地/目的地」的來回票，
沒有辦法表達「進 A 出 B」這種開口式行程。

繞過辦法：把每一段拆成獨立的單程查詢（各自都是 fast-flights 查得動的正常查
詢），把最便宜的單程票價加起來當作總價。**這不是航空公司賣的聯程票價**——
聯程票有時候比兩張單程票加起來便宜，所以這個數字只能當參考、抓個大概範
圍，實際訂票前還是要去 Google Flights 或 Kiwi 手動查證比較。通知裡的訂票
連結也會是兩個各自單程的連結，不是一個聯程訂票連結。

> ⚠️ 注意：一整個目的地全數失敗時，Actions 仍然是綠的，
> 因為只要還有別的路線查到價格就不算整輪失敗。
> 要確認某個目的地有沒有真的抓到資料，看 `data/prices.csv` 裡有沒有它的列。

**`/chart` 需要 matplotlib，這個套件刻意沒放進基本安裝**（`pyproject.toml`
的 `chart` extra）——`commands.yml` 每一則指令都要跑 `pip install`，
matplotlib 拖著 numpy 一整串依賴，裝進每一則指令會拖慢原本很快的 webhook
回應。`commands.yml` 偵測到訊息包含 `/chart` 才會多裝 `matplotlib` 和一套
中文字型 `fonts-wqy-zenhei`，其餘指令維持原本的安裝速度。

**畫出來的圖標題、座標軸都是中文，matplotlib 內建字型完全沒有中文字**，
沒裝中文字型的話會整片畫成空白方塊，這也是上面要多裝 `fonts-wqy-zenhei`
的原因。本機測試（`pip install -e ".[chart]"` 之後跑 `flight-tracker chart`）
的話，系統要有裝任何一種中文字型（`fc-list | grep -i noto` 之類的指令可以
確認）；沒有的話圖還是畫得出來，只是文字會是空白方塊。
