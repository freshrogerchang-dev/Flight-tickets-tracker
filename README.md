...
`/add` `/rm` 是改**一條路線裡的目的地**；`/newroute` `/newmulti` `/delroute`
是新增/刪除**整條路線**，兩組指令的作用範圍不一樣，別搞混了。`/newroute`
只能建立來回行程（沿用 `defaults` 的轉機上限等設定）；多段行程（不同城市
進出）要用 `/newmulti`，每一段用 `出發地>目的地@日期` 表示、中間用空格分開，
至少要兩段；單程還是得直接編輯 `routes.yaml`。
